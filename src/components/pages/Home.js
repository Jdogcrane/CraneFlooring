...
export default function Home() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) {
        setVideoSrc('https://www.youtube.com/embed/lOTXXelNW30');
        observer.disconnect();
      }
    }, { rootMargin: '200px' });
  ...
  const onMouseDown = (e) => { isDown = true; startX = e.pageX - sliderRef.current.offsetLeft; scrollLeft = sliderRef.current.scrollLeft; };
  const onMouseLeave = () => { isDown = false; };
  const onMouseUp = () => { isDown = false; };
  const onMouseMove = (e) => { if (!isDown) return; e.preventDefault(); const x = e.pageX - sliderRef.current.offsetLeft; sliderRef.current.scrollLeft = scrollLeft - (x - startX); };

  return (
    <>
      <section id="parallax-1" className="center">
        <img ref={heroRef} src={require('../images/hero.jpeg')} alt="Crane Flooring" className="hero-img" />
        <div className="hero-overlay animate__animated animate__fadeIn animate__delay-1s">
          <div className="hero-text">
            <p className="caption white-text background-color">Crane Flooring</p>
            <p className="normalText white-text italic">&ldquo;Excellence in every step&rdquo;</p>
          </div>
        </div>
      </section>
      ...
      <div className="video-cta-card contain1">
        <div className="video-wrapper" ref={videoRef}>
          {videoSrc && <iframe className="videoSizing" src={videoSrc} title="YouTube video player" frameBorder="0" allow="accelerometer; encrypted-media; gyroscope; web-share; fullscreen"></iframe>}
        </div>
        <div className="cta-strip">
          <p className="cta-label">Ready to get started?</p>
          <div className="cta-buttons">
            <NavLink to="/gallery" onClick={() => window.scrollTo(0, 0)} className="btn-custom">View Our Work</NavLink>
            <NavLink to="/contact" onClick={() => window.scrollTo(0, 0)} className="btn-custom btn-custom-fill">Set Appointment</NavLink>
          </div>
        </div>
      </div>
      ...
      <div className="contain1 spaceSmaller">
        <div className="center">
          <a className="google-badge" rel="noreferrer" href="https://www.google.com/search?q=crane+hardwood+flooring#lrd=0x89baa75c066f7f01:0x8ca1022771311150,1" target="_blank">
      ...
      <div className="contain1 space spaceDown">
        <div className="about-header spacing2">
          ...
          <div className="about-socials">
            <a href="https://www.instagram.com/cranehardwoodflooring/?hl=en" rel="noreferrer" target="_blank">
              <i className="fab fa-instagram"></i>
            </a>
            <a href="https://www.facebook.com/craneflooring" rel="noreferrer" target="_blank">
        ...
        <div className="about-box spaceSmaller">
          ...
          <p className="normalText" style={{marginTop: '1rem'}}>
            Since then Crane Flooring has provided hardwood flooring services to over 200+ customers. We service the greater Hampton Roads area — Virginia Beach, Norfolk, Chesapeake, Suffolk, Hampton, Newport News, and Isle of Wight County. We take pride in providing excellent flooring services at an affordable price.
          </p>
        </div>
        <div className="about-promise spaceSmaller">
          <p className="greyText">Our Promise</p>
          <p className="normalText" style={{marginTop: '0.75rem'}}>
